...
import { getSupplyChains } from 'apiSdk/supply-chains';
...
function DashboardPage() {
  ...
  const supplyChainStat = useSWR<PaginatedInterface<SupplyChainInterface>>(
    () => `/supply-chains`,
    () => getSupplyChains({ limit: 1, offset: 0 }),
  );
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    ...
    {
      title: 'Supply Chains',
      result: supplyChainStat?.data?.totalCount,
