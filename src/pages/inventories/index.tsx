...
export function InventoryListPage(props: InventoryListPageProps) {
  ...
  const fetcher = useCallback(
    async () =>
      getInventories({
        relations: ['business', 'sales.count', 'supply_chain.count'],
        limit: params.pageSize,
        offset: params.pageNumber * params.pageSize,
        searchTerm: params.searchTerm,
        order: params.order,
        searchTermKeys: ['product_name.contains'],
        ...(params.filters || {}),
      }),
  ...
  const columns: ColumnType[] = [
    ...
    { id: 'quantity', header: 'quantity', accessorKey: 'quantity' },
    {
      ...
      cell: ({ row: { original: record } }: any) =>
        record?.last_restock_date ? format(parseISO(record?.last_restock_date as unknown as string), 'dd-MM-yyyy') : '',
    },
    {
      id: 'next_restock_date',
      header: 'Next Restock Date',
      accessorKey: 'next_restock_date',
      cell: ({ row: { original: record } }: any) =>
        record?.next_restock_date ? format(parseISO(record?.next_restock_date as unknown as string), 'dd-MM-yyyy') : '',
    },
